...
static char* guess_content_type_info(struct FileInfo *info);
...
read_header_field(FILE *in)
{
    ...
    if (!fgets(buf, LINE_BUF_SIZE, in)) {
        log_exit("failed request header field: %s", strerror(errno));
    }
    if ((buf[0] == '\n') || (strcmp(buf, "\r\n") == 0)) {
        return NULL;
    }
...
static char*
guess_content_type_info(struct FileInfo *info)
{
    return "text/plain";
}
